...
if (require.main === module) {
  (async () => {
    const parser = yargs(hideBin(process.argv)).strict().options({
      reportDir: {
        type: "string",
        demandOption: true,
        description: "directory where output files ('report.json', 'prompts.json') are placed",
      }
    });
    const { reportDir } = await parser.argv;
    const tests: TestReport[] = JSON.parse(fs.readFileSync(`${reportDir}/report.json`, "utf8")).tests;
    const testsMap = new Map<string, TestReport>();
    for (const test of tests) {
      testsMap.set(test.testName, test);
    }
    const prompts: PromptReport[] = JSON.parse(fs.readFileSync(`${reportDir}/prompts.json`, "utf8")).prompts;
    ...
    for (const prompt of prompts) {
      const testResults = prompt.tests.map(test => {
        ...
        else if (testsMap.get(test)!.status === "FAILED" && testsMap.get(test)!.err.stack?.startsWith("AssertionError")) return "AssertionFailed";
        else return "FAILED";
      });
    ...
    for (const prompt of prompts) {
      for (const refiner of new Set(prompt.provenance.map(p => p.refiner))) {
        if (improvements.summary.find(i => i.refiner === refiner) === undefined) {
        ...
        const existingSummary = improvements.summary.find(i => i.refiner === refiner);
        const existingInstance = improvements.instances.find(i => i.refiner === refiner);
        if (promptSuccess.get(prompt.id) === "PASSED") {
          if (prompt.provenance.every(pr => promptSuccess.get(pr.originalPrompt) !== "PASSED")) {
          ...
          if (prompt.provenance.every(pr => promptSuccess.get(pr.originalPrompt) === "FAILED")) {
    ...
    fs.writeFileSync(`${reportDir}/improvements.json`, JSON.stringify(improvements, null, 2));
  })().catch((err) => {
    console.error(err);
    process.exit(1);
  });
